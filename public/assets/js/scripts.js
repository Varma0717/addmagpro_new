...
$(function () {
  ...
  $('.main-sidebar .sidebar-menu a[href]').each(function () {
    var linkUrl = this.href ? this.href.split('#')[0].replace(/\/+$/, '') : '';

    if (!linkUrl || linkUrl === window.location.origin || linkUrl !== currentUrl) {
      return;
    }

    var $link = $(this);
    var $item = $link.closest('li');
    var $parentMenu = $link.closest('.dropdown-menu');
    ...
    if ($parentMenu.length) {
      $parentMenu.show();
      $parentMenu.closest('li.dropdown').addClass('active');
      $parentMenu.prev('a.has-dropdown').addClass('toggled');
    }
  });
