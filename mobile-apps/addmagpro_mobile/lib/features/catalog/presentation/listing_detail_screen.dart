...
class _ListingDetailScreenState extends State<ListingDetailScreen> {
  ...
  void _share() {
    ...
    SharePlus.instance.share(
      ShareParams(text: 'Check out ${listing.businessName} on AddMagPro!'),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_titleForType(listing?.type)),
        actions: [
          if (listing != null)
            IconButton(
              onPressed: _share,
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          if ((listing.phone ?? '').isNotEmpty &&
                              ((listing.whatsapp ?? '').isNotEmpty ||
                                  (listing.websiteUrl ?? '').isNotEmpty))
                            const SizedBox(width: 10),
  ...
  bool _hasAnyContact(ListingDetail listing) {
    return (listing.phone ?? '').isNotEmpty ||
        (listing.whatsapp ?? '').isNotEmpty ||
        (listing.websiteUrl ?? '').isNotEmpty;
  }
