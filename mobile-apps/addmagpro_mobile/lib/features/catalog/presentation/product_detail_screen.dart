...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  Future<void> _initializeWishlistState(int productId) async {
    ...
    try {
      final inWishlist = await _wishlistRepository.check(
        token: token,
        productId: productId,
      );
      if (!mounted) return;
  ...
  void _share() {
    ...
    SharePlus.instance.share(
      ShareParams(
        text:
            'Check out ${product.name} on AddMagPro! ₹${product.effectivePrice.toStringAsFixed(0)}',
      ),
    );
  }
  ...
  Future<void> _toggleWishlist() async {
    ...
    if (token == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Login required to manage wishlist'),
          behavior: SnackBarBehavior.floating,
        ),
      );
      return;
    ...
    try {
      final inWishlist = await _wishlistRepository.toggle(
        token: token,
        productId: product.id,
      );
      if (!mounted) return;
